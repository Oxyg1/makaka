...
export type OrderKind = 'trade' | 'sell' | 'any';
export type OrderStatus = 'open' | 'locked' | 'done' | 'cancelled';
...
export interface MarketEntry extends Frog {
  // overrides since /market joins differently
  id: number;          // order id !
  frog_id: number;
  kind: OrderKind;
  price_stars: number | null;
...
export type OfferStatus = 'pending' | 'accepted' | 'declined' | 'cancelled' | 'awaiting_escrow' | 'completed' | 'expired';
...
export interface OfferDirection {
  ...
  message: string | null;
  status: OfferStatus;
  from_escrow_at: string | null;
  to_escrow_at: string | null;
